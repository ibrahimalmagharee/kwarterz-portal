            ...
            <div class="container">
                <div class="row justify-content-center">
                    <div class="page-title style1 col-xl-6 col-lg-8 col-md-10 text-center mb-5">
                        <h2 class="text-grey-900 fw-700 font-xxl pb-3 mb-0 mt-3 d-block lh-3">جميع الدورات المحفوظة</h2>
                    </div>
                </div>
                <div class="row">
                        ...
                        <div class="col-xl-4 col-xxxl-3 col-lg-6 col-md-6 col-sm-6 mb-4">
                            <div class="card w-100 p-0 shadow-xss border-0 rounded-lg overflow-hidden mr-1">
                                <div class="card-image w-100 mb-3">
                                    @if ($course->course->is_purchased(\Auth::user()->id))
                                        ...
                                        @else
                                        <a href="{{route('user.show.course', $course->course->slug)}}" class="position-relative d-block"><img src="{{$course->course->image_path}}" alt="image" class="w-100"></a>
                                    @endif
                                </div>
                                <div class="card-body pt-0">
                                    ...
                                    @if ($course->course->is_purchased(\Auth::user()->id))
                                        ...
                                        @else
                                        <h4 class="fw-700 font-xss mt-3 lh-28 mt-0"><a href="{{route('user.show.course', $course->course->slug)}}" class="text-dark text-grey-900">{{$course->course->title}} </a></h4>
                                    @endif
                    ...
                    @endforeach
                    @if (count($courses) == 0)
                        <h3 class="text-info mr-2">لم تقم بحفظ أي كورس بعد</h3>
                    @endif
                </div>
            </div>
        </div>
